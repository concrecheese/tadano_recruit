...
!(function (e, t) {
	...
	var l = {
		...
		offset: function () {
			if (this.length > 0) {
				...
				var s = i.body;
				var r = e.clientTop || s.clientTop || 0;
				var n = e.clientLeft || s.clientLeft || 0;
				var o = e === a ? a.scrollY : e.scrollTop;
				var l = e === a ? a.scrollX : e.scrollLeft;
		...
		children: function (e) {
			for (var t = [], i = 0; i < this.length; i += 1)
				for (var s = this[i].childNodes, a = 0; a < s.length; a += 1)
					e
						? s[a].nodeType === 1 && n(s[a]).is(e) && t.push(s[a])
						: s[a].nodeType === 1 && t.push(s[a]);
			return new r(o(t));
		},
		filter: function (e) {
			for (var t = [], i = 0; i < this.length; i += 1)
				...
				a.WebKitCSSMatrix
					? ((s = n.transform || n.webkitTransform).split(',').length > 6 &&
							(s = s
								...
								.join(', ')),
					  (r = new a.WebKitCSSMatrix(s === 'none' ? '' : s)))
					: (i = (r =
							n.MozTransform ||
							n.OTransform ||
							n.MsTransform ||
	...
	var c = { components: { configurable: !0 } };
	...
	var u = {
		...
		updateSlides: function () {
			...
			var r = this.wrongRTL;
			var n = this.virtual && e.virtual.enabled;
			var o = n ? this.virtual.slides.length : this.slides.length;
			var l = t.children('.' + this.params.slideClass);
			var h = n ? this.virtual.slides.length : l.length;
			var p = [];
			var c = [];
			var u = [];
			function v(t) {
				...
				for (
					...
					if (e.slidesPerColumn > 1) {
						...
						if (e.slidesPerColumnFill === 'row' && e.slidesPerGroup > 1) {
							...
							var A = k - e.slidesPerColumn * e.slidesPerGroup * O;
							var G =
								O === 0
									? e.slidesPerGroup
									: Math.min(
					...
					if ($.css('display') !== 'none') {
						if (e.slidesPerView === 'auto') {
							...
							var B = $[0].style.transform;
							var N = $[0].style.webkitTransform;
							if (
								...
								var V = parseFloat(H.getPropertyValue('padding-left'));
								var Y = parseFloat(H.getPropertyValue('padding-right'));
								var F = parseFloat(H.getPropertyValue('margin-left'));
								var W = parseFloat(H.getPropertyValue('margin-right'));
								var R = H.getPropertyValue('box-sizing');
								S = R && R === 'border-box' ? X + F + W : X + V + Y + F + W;
							} else {
								var q = parseFloat(H.getPropertyValue('height'));
								var j = parseFloat(H.getPropertyValue('padding-top'));
								var K = parseFloat(H.getPropertyValue('padding-bottom'));
								var U = parseFloat(H.getPropertyValue('margin-top'));
								var _ = parseFloat(H.getPropertyValue('margin-bottom'));
								var Z = H.getPropertyValue('box-sizing');
								S = Z && Z === 'border-box' ? q + U + _ : q + j + K + U + _;
							}
			...
			if (i.length !== 0) {
				...
				for (var r = 0; r < i.length; r += 1) {
					...
					if (t.watchSlidesVisibility || (t.centeredSlides && t.autoHeight)) {
						...
						var h = d + this.slidesSizesGrid[r];
						((d >= 0 && d < this.size - 1) ||
							(h > 1 && h <= this.size) ||
							(d <= 0 && h >= this.size)) &&
		...
		updateSlidesClasses: function () {
			...
			var r = this.realIndex;
			var n = this.virtual && i.virtual.enabled;
			t.removeClass(
				i.slideActiveClass +
					' ' +
					i.slideNextClass +
		...
		updateActiveIndex: function (e) {
			var t;
			var i = this.rtlTranslate ? this.translate : -this.translate;
			var s = this.slidesGrid;
			var a = this.snapGrid;
			var r = this.params;
			var n = this.activeIndex;
			var o = this.realIndex;
			var l = this.snapIndex;
			var h = e;
			if (void 0 === h) {
		...
		updateClickedSlide: function (e) {
			var t = this.params;
			var i = n(e.target).closest('.' + t.slideClass)[0];
			var s = !1;
			if (i)
	...
	var v = {
		...
		translateTo: function (e, t, i, s, a) {
			...
			var d;
			var h = n.minTranslate();
			var p = n.maxTranslate();
			if (
				((d = s && e > h ? h : s && e < p ? p : e),
				n.updateProgress(d),
				o.cssMode)
	...
	var m = {
		slideTo: function (e, t, i, s) {
			...
			var l = r.snapGrid;
			var d = r.slidesGrid;
			var h = r.previousIndex;
			var p = r.activeIndex;
			var c = r.rtlTranslate;
			var u = r.wrapperEl;
		...
		slideToClosest: function (e, t, i, s) {
			void 0 === e && (e = this.params.speed),
				...
				void 0 === s && (s = 0.5);
			var a = this.activeIndex;
			var r = Math.min(this.params.slidesPerGroupSkip, a);
			var n = r + Math.floor((a - r) / this.params.slidesPerGroup);
	...
	var g = {
		...
		loopFix: function () {
			...
			var i = this.slides;
			var s = this.loopedSlides;
			var a = this.allowSlidePrev;
	...
	function G(e) {
		...
		if (!this.animating || !s.preventInteractionOnTransition) {
			...
			if (
				...
				((t.isTouchEvent = o.type === 'touchstart'),
				(t.isTouchEvent || !('which' in o) || o.which !== 3) &&
					!(
				...
				if (
					...
					var h = r.currentX;
					var p = r.currentY;
					var c = s.edgeSwipeDetection || s.iOSEdgeSwipeDetection;
					var u = s.edgeSwipeThreshold || s.iOSEdgeSwipeThreshold;
	...
	function H(e) {
		...
		if ((o.originalEvent && (o = o.originalEvent), t.isTouched)) {
			if (!t.isTouchEvent || o.type === 'touchmove') {
				...
				var h = 'touchmove' === o.type ? l.pageX : o.pageX;
				var p = o.type === 'touchmove' ? l.pageY : o.pageY;
		...
		if (
			...
			if (s.freeMode) {
				...
				if (s.freeModeMomentum) {
					...
					var E;
					var T = !1;
					var S = 20 * Math.abs(t.velocity) * s.freeModeMomentumBounceRatio;
					if (y < t.maxTranslate())
					...
					if (
						...
						if (
							...
							var z = t.slidesSizesGrid[t.activeIndex];
				...
				var D = (p - o[k]) / $;
				var O = k < s.slidesPerGroupSkip - 1 ? 1 : s.slidesPerGroup;
				if (u > s.longSwipesMs) {
					...
					(h.target === t.navigation.nextEl || h.target === t.navigation.prevEl)
						? h.target === t.navigation.nextEl
							? t.slideTo(k + O)
							: t.slideTo(k)
						: (t.swipeDirection === 'next' && t.slideTo(k + O),
	...
	function V() {
		...
		(i === 0 ? 0 : (this.translate - this.minTranslate()) / i) !==
			this.progress &&
	...
	var K = { name: 'device', proto: { device: A }, static: { device: A } };
	var U = { name: 'support', proto: { support: h }, static: { support: h } };
	var _ = {
	...
	var re = {
		...
		handle: function (e) {
			...
			var a = i.$el;
			if (
				(i.params.mousewheel.eventsTarged !== 'container' &&
			...
			var r = 0;
			var o = i.rtlTranslate ? -1 : 1;
			var l = re.normalize(t);
			if (s.forceToAxis)
				if (i.isHorizontal()) {
			...
			if ((s.invert && (r = -r), i.params.freeMode)) {
				...
				var p = i.mousewheel.lastEventBeforeSnap;
				var c =
					p &&
					h.time < p.time + 500 &&
					h.delta <= p.delta &&
				...
				var x = i.mousewheel.recentWheelEvents;
				x.length >= 2 && x.shift();
			...
			d.shadow &&
				(h
					? (0 === (e = i.find('.swiper-cube-shadow')).length &&
							((e = n('<div class="swiper-cube-shadow"></div>')), i.append(e)),
					  e.css({ height: a + 'px' }))
		...
		onThumbClick: function () {
			...
			if (e) {
				...
				if (
					...
					if (
						((s = e.params.loop
							? parseInt(n(e.clickedSlide).attr('data-swiper-slide-index'), 10)
							: t),
						this.params.loop)
					) {
						var a = this.activeIndex;
...
var craneSlide = new Swiper('.crane_swiper', {
	speed: 2000,
	loop: true,
	effect: 'slide',
	spaceBetween: 0,
...
var productThumbs = new Swiper('.product_infoSwiper', {
	speed: 600,
	loop: false,
	effect: 'fade',
	slidesPerView: 1,
